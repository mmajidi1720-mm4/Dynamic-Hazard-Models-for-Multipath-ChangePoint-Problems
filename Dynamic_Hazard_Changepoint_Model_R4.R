...
library(parallel)
# ------------------------------------------------------------------------------
# Set RUN_PRACTICAL_EXAMPLE = TRUE to run the minimal self-contained user example.
...
neg_log_likelihood_general <- function(theta, X, Z) {
  # Robustness: ensure matrix inputs (handles vector Z when p = 1)
  X <- as.matrix(X)
  Z <- as.matrix(Z)
  
  if (anyNA(X) || anyNA(Z)) {
    stop("X and Z must not contain missing values (NA). Balanced complete data assumed.")
  }
  ...
  if (nrow(Z) != n) {
    stop("Number of rows of Z must equal number of rows of X.")
  }
...
# ------------------------------------------------------------------------------
# 2. Main Fitting Function (with simple multi-start for practical robustness)
# ------------------------------------------------------------------------------
fit_dhcm <- function(X, Z, n_starts = 5) {
  # Robustness: ensure matrix inputs (handles vector Z when p = 1)
  X <- as.matrix(X)
  Z <- as.matrix(Z)
  ...
  # Smart Initialization (primary start)
  init_beta <- rep(0, p)
  init_psi <- 0
  init_mu0 <- mean(X[, 1:max(1, floor(m/2))], na.rm = TRUE)
  init_log_sigma0 <- log(sd(X[, 1:max(1, floor(m/2))], na.rm = TRUE) + 0.1)
  init_mu1 <- mean(X[, max(1, floor(m/2)):m], na.rm = TRUE)
  init_log_sigma1 <- log(sd(X[, max(1, floor(m/2)):m], na.rm = TRUE) + 0.1)
  
  theta_smart <- c(init_beta, init_psi, init_mu0, init_log_sigma0, init_mu1, init_log_sigma1)
  
  # Simple multi-start: smart init + a few random perturbations around it
  # (helps avoid poor local maxima on real data; still lightweight for practical use)
  set.seed(123)  # for reproducibility of the random starts inside the function
  starts <- list(theta_smart)
  if (n_starts > 1) {
    for (s in 2:n_starts) {
      pert <- rnorm(length(theta_smart), mean = 0, sd = c(rep(0.3, p), 0.02, 0.3, 0.2, 0.3, 0.2))
      starts[[s]] <- theta_smart + pert
    }
  }
  ...
  for (s in seq_along(starts)) {
    ...
    if (!is.null(opt_result)) {
      # Prefer converged solutions; among them (or all) keep the lowest negative log-likelihood
      if (is.null(best_opt) || 
          (opt_result$convergence == 0 && (best_opt$convergence != 0 || opt_result$value < best_val)) ||
          (opt_result$convergence != 0 && best_opt$convergence != 0 && opt_result$value < best_val)) {
  ...
  est <- best_opt$par
  se <- tryCatch({
    invH <- solve(best_opt$hessian)
    if (any(diag(invH) <= 0)) {
      warning("Hessian is not positive definite; standard errors set to NA.")
      rep(NA_real_, length(est))
    } else {
      sqrt(diag(invH))
    }
  }, error = function(e) {
    warning("Could not invert Hessian; standard errors set to NA.")
    rep(NA_real_, length(est))
  })
  
  # Delta method for SE of sigma = exp(log_sigma)
  sigma0_se <- if (!is.na(se[p + 3])) se[p + 3] * exp(est[p + 3]) else NA_real_
  sigma1_se <- if (!is.na(se[p + 5])) se[p + 5] * exp(est[p + 5]) else NA_real_
  ...
  res <- list(
    ...
    sigma0_se = sigma0_se,          # delta-method SE for sigma0
    mu1_se = se[p + 4],
    log_sigma1_se = se[p + 5],
    sigma1_se = sigma1_se,          # delta-method SE for sigma1
  ...
  if (res$convergence != 0) {
    warning("Optimization did not fully converge (convergence code = ", res$convergence,
            "). Results may be unreliable; consider more starts or different initial values.")
  }
...
# ------------------------------------------------------------------------------
# 3. Posterior Changepoint Estimation (MAP estimate of changepoint)
# ------------------------------------------------------------------------------
predict_changepoints <- function(fit_obj, X, Z) {
  ...
  # MAP estimates of individual changepoints
...
if (RUN_PRACTICAL_EXAMPLE) {
  ...
  fit_results <- fit_dhcm(X = X_real, Z = Z_real, n_starts = 5)
  
  # Explicit convergence check for the practical example
  if (fit_results$convergence != 0) {
    warning("Practical example: optim did not fully converge (code = ", 
            fit_results$convergence, "). Interpret estimates with caution.")
  } else {
  ...
  cat("Corresponding SEs:\n")
  print(round(fit_results$beta_se, 4))
  
  cat("Time Effect (Psi):\n")
  print(round(fit_results$psi_hat, 4))
  cat("SE(Psi):\n")
  print(round(fit_results$psi_se, 4))
  
  cat("Pre-change Mean (mu0) & SD (sigma0):\n")
  cat(sprintf("mu0: %.4f (SE = %.4f) | sigma0: %.4f (SE = %.4f)\n", 
              fit_results$mu0_hat, fit_results$mu0_se,
              fit_results$sigma0_hat, fit_results$sigma0_se))
  cat("Post-change Mean (mu1) & SD (sigma1):\n")
  cat(sprintf("mu1: %.4f (SE = %.4f) | sigma1: %.4f (SE = %.4f)\n", 
              fit_results$mu1_hat, fit_results$mu1_se,
              fit_results$sigma1_hat, fit_results$sigma1_se))
  cat(sprintf("Log-Likelihood: %.2f\n", fit_results$logLik))
  cat(sprintf("Number of multi-starts used: %d\n", fit_results$n_starts_used))
  
  estimated_tau <- predict_changepoints(fit_results, X = X_real, Z = Z_real)
  
  cat("\n--- Changepoint Estimates (MAP) ---\n")
...
if (RUN_SIMULATION) {
  ...
  cat("\nMonte Carlo simulation skipped (RUN_SIMULATION = FALSE).\n")
  cat("Set RUN_SIMULATION <- TRUE (and optionally FAST_MODE <- TRUE) to run the full simulation study.\n")
}
